...
function validarEstudio() {
    ...
    var af_conectividad = document.forms["revision_ssr"]['af_conectividad'].value;
    var af_fttn = document.forms["revision_ssr"]['af_fttn'].value;
    var af_otros = document.forms["revision_ssr"]['af_otros'].value;
	...
	if (estado == 'Pendiente CAB'){
        ...
        if (af_conectividad == null || af_conectividad == 0) {
            alert("Debe seleccionar una opción del desplegable AF CONECTIVIDAD");
            return false;
        }

        if (af_fttn == null || af_fttn == 0) {
            alert("Debe seleccionar una opción del desplegable AF FTTN");
            return false;
        }

        if (af_otros == null || af_otros == 0) {
            alert("Debe seleccionar una opción del desplegable AF OTROS");
            return false;
        }
